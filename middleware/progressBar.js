...
const predictions = (totalFrames) => {
    const progressPredictions = new cliProgress.SingleBar({
        format: 'Processing detect nine dash |' + colors.cyan('{bar}') + '| {percentage}% || {value}/{total} Frames',
        barCompleteChar: '\u2588',
        barIncompleteChar: '\u2591',
        hideCursor: true
    });
...
const render = (totalFrames) => {
    const progressRender = new cliProgress.SingleBar({
        format: 'Processing mapping predictions |' + colors.cyan('{bar}') + '| {percentage}% || {value}/{total} Frames',
        barCompleteChar: '\u2588',
        barIncompleteChar: '\u2591',
        hideCursor: true
    });
...
const frames = (totalFrames) => {
    const progressFrames = new cliProgress.SingleBar({
        format: 'Rendering detected image |' + colors.cyan('{bar}') + '| {percentage}% || {value}/{total} Frames',
        barCompleteChar: '\u2588',
        barIncompleteChar: '\u2591',
        hideCursor: true
    });
